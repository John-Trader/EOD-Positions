...
try {
  ...
  assert(snap.version === '3.8.0', 'portable version synced');
  ...
  assertThrows(() => api.prepareBackup({ version: '3.8.0', scannerStores: { bogus: {} } }), 'unknown scanner store rejected');
  assertThrows(() => api.prepareBackup({ version: '3.8.0', activeTradesLog: [{ id: 1, ticker: 'XX', side: 'LONG', status: 'ACTIVE', shares: 'lots' }] }), 'malformed trade rejected');
